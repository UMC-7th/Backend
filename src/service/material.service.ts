import { deleteMarkMaterial, getMarkMaterial, getMarkMaterialList, markMaterial } from "../repository/material.repository.js";
...
//식재료 북마크 삭제
export const deleteMarkMaterialService = async (userId: number, material: string) => {
    const confirm = await getMarkMaterial(userId, material);
    if (!confirm) {
        throw new InvalidInputError("북마크된 식재료에 존재하지 않습니다.", `userId: ${userId}, material: ${material}`);
    }
